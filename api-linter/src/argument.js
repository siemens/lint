...
exports.argument = () => {
  ...
  program
    ...
    .addOption(
      new Option('-c, --consoleSeverity <console severity>', 'console output severity').default(
        'warn'
      )
    )
    .addOption(
      new Option('-o, --outputFilename <output filename>', 'output filename').default(
        'linter-result.html'
      )
    )
    .addOption(
      new Option('-p, --jsonFile <output json file>', 'output json file').default(
        'spectral_result.json'
      )
    )
    .addOption(
      new Option('-v, --apiVersioning <api versioning>', 'api versioning')
        .choices(['ignore', 'url', 'header'])
        .default('ignore')
    )
    .addOption(
      new Option('-a, --apiSecurity <api security>', 'api security')
