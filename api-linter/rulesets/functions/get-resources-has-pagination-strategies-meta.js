...
const followCursorBasedStrategies = requestQueryParameters => {
  return (
    requestQueryParameters.filter(param => param.name == 'cursor' || param.name == 'limit')
      .length == 2
  );
};
...
const followOffsetBasedStrategies = requestQueryParameters => {
  return (
    requestQueryParameters.filter(param => param.name == 'offset' || param.name == 'limit')
      .length == 2
  );
};
...
const followIndexBasedStrategies = requestQueryParameters => {
  return (
    requestQueryParameters.filter(param => param.name == 'number' || param.name == 'size').length ==
    2
  );
};
...
const checkIndexMeta = meta => {
  return (
    jp.query(meta, '$.properties.page.properties.totalPages').length > 0 &&
  ...
  if (responseSchema.length > 0) {
    ...
    if (isCollectionResponse(schema) && hasTopLink(schema)) {
      ...
      if (linkProps.filter(prob => hasPageLinkKeys(prob)).length > 0) {
        ...
        if (
          options == 'cursor' &&
          requestQueryParameters.length > 0 &&
          followCursorBasedStrategies(requestQueryParameters)
        ) {
          if (responseMeta.length == 0 || !checkCursorMeta(responseMeta[0])) {
            return [
              {
                message:
                  'The server MAY provide information about the nextCursor that is required to fetch the next page in the meta.page.nextCursor property'
              }
            ];
          }
        }
        if (
          options == 'offset' &&
          requestQueryParameters.length > 0 &&
          followOffsetBasedStrategies(requestQueryParameters)
        ) {
          if (responseMeta.length == 0 || !checkOffsetMeta(responseMeta[0])) {
            return [
              {
                message:
                  'Server SHOULD provide pagination meta [meta.page.totalElements, meta.page.offset, meta.page.elements] information to the client'
              }
            ];
          }
        }
        if (
          options == 'index1' &&
          requestQueryParameters.length > 0 &&
          followIndexBasedStrategies(requestQueryParameters)
        ) {
          if (responseMeta.length == 0 || !checkIndexMeta(responseMeta[0])) {
            return [
              {
                message:
                  'Server SHOULD provide pagination meta [meta.page.totalPages, meta.page.number, meta.page.size] information to the client'
              }
            ];
          }
        }
        if (
          ...
          if (
            ...
            (checkIndexMeta(responseMeta[0]) && !checkIndexMeta2(responseMeta[0]))
          ) {
            return [
              {
                message:
                  'Server SHOULD provide pagination meta [meta.page.elements, meta.page.totalElements] information to the client'
              }
            ];
          }
        }
