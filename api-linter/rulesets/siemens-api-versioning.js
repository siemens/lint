...
const hasPropertyValue = (targetVal, options) => {
  ...
  if (!exist) {
    results.push({
      message:
        'A request header Api-Version MUST be supported to allow client to specify the version.'
    });
  }
  return results;
};
...
const hasProperty = (targetVal, options) => {
  ...
  if (typeof targetVal['headers'] == 'undefined') {
    ...
    results.push({
      message:
        "A header with full semantic version value MUST be returned in the response with 'Api-Version: <MAJOR>.<MINOR>.<PATCH>'"
    });
  }
  return results;
};
  ...
  case 'url':
    ruleset.rules = {
      'Siemens-API-[200.1]': {
        ...
        documentationUrl:
          'https://developer.siemens.com/guidelines/api-guidelines/rest/versioning.html#versioningrule200.1',
        severity: 'error',
        given: ['$.servers..url'],
        then: {
          function: pattern,
  ...
  case 'header':
    ruleset.rules = {
      'Siemens-API-[200.2]': {
        message: '{{message}}',
        description:
          'A request HTTP header e.g., Api-Version Should be supported to allow client to specify the version.',
        documentationUrl:
          'https://developer.siemens.com/guidelines/api-guidelines/rest/versioning.html#versioningrule200.2',
        severity: 'error',
...
if (apiVersioning == 'url' || apiVersioning == 'header') {
  ruleset.rules['Siemens-API-[201]'] = {
    message: '{{message}}',
    description:
      "A header with full semantic version value SHOULD be returned in the response with 'Api-Version: <MAJOR>.<MINOR>.<PATCH>'",
    documentationUrl:
      'https://developer.siemens.com/guidelines/api-guidelines/rest/versioning.html#versioningrule201',
    severity: 'warn',
